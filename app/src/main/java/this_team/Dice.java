...
public class Dice extends BranchGroup {
    ...
    static final float TABLE_HEIGHT = 0.25f;
    ...
    // Public getter to return the upward (settled) face of the dice.
    public int getSettledFace() {
        return findClosestFaceToVector(new Vector3f(0, 1, 0));
    }
    
    // Public method to check if the dice is still rolling or settling.
    public boolean isDiceRolling() {
        return isRolling || isSettling;
    }
    ...
    public static void main(String[] args) {
        ...
        simulateBtn.addActionListener(e -> {
            new Thread(() -> { dice.simulate(); }).start();
        });
