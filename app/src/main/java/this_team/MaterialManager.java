...
public class MaterialManager {
	...
	public static Material highlight_Material(Color3f m_clr) {
		Material mtl = new Material();
		mtl.setShininess(152);
		mtl.setAmbientColor( White);
		mtl.setDiffuseColor(m_clr);
		mtl.setSpecularColor(Grey);
		Color3f emissiveColor = new Color3f(m_clr);
		emissiveColor.scale(0.4f);
		mtl.setEmissiveColor(emissiveColor);
		mtl.setLightingEnable(true);
		return mtl;
	}
	...
	/* a function to set appearance with provided color ('clr') and predefined material */
	public static Appearance highlight_Appearance(Color3f clr) {
		Appearance app = new Appearance();
		app.setMaterial(highlight_Material(clr));                // set appearance's material
		return app;
	}
